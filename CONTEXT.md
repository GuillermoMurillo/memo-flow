...
- **Memo-flow-managed mutation**: any change made by `setup-memo-flow` or `install-memo-hooks`. Each must be recorded in the installed manifest so it can be inspected by the doctor skill and reversed by uninstall.
